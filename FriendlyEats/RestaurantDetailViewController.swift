...
//  https://codelabs.developers.google.com/codelabs/firestore-ios/#8
//
...
class RestaurantDetailViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, NewReviewViewControllerDelegate {
    ...
    func reviewController(_ controller: NewReviewViewController, didSubmitFormWithReview review: Review) {
        ...
        firestore.runTransaction({ (transaction, errorPointer) -> Any? in
            ...
            guard let restaurantData = restaurantSnapshot.data() else {
                print("Error in retrieving restaurantSnapshot.data()")
                return nil
            }
            guard let restaurant = Restaurant(dictionary: restaurantData) else {
